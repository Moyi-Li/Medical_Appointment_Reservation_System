...
class Patient:
    ...
    def patient_appointment_info(self):
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor(as_dict=True)

        check_availability = "SELECT * FROM Appointments WHERE Patient_name = %s ORDER BY AppointmentID"
        try:
            cursor.execute(check_availability, self.username)
            found_appointments = False
            print("AppointmentID Vaccine_name Date Caregiver_username:")

            for row in cursor:
                curr_id = row['AppointmentID']
                curr_vacc = row['Vaccine_name']
                curr_date = row['Time']
                curr_caregiver = row['Caregiver_name']
                print(f"{curr_id} {curr_vacc} {curr_date} {curr_caregiver}")
